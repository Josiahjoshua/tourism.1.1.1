        ...
        <div class="article">
            ...
            @foreach (json_decode($publication['file']) as $file)
                ...
                <div class="row align-content-center mt-3">
                    <a href="{{ asset('storage/'.$file->download_link) }}" class="ml-3 link-no-underline" target="_blank"><i class="fa fa-eye"></i> Tazama</a>
                    <a href="{{ asset('storage/'.$file->download_link) }}" class="ml-5 link-no-underline" download="{{ $file->original_name}}"><i class="fa fa-download"></i> Pakua</a>
                </div>
